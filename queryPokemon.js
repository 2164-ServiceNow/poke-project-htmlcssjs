...
function updateCard(pokemon) {
  ...
  pokeName.innerHTML += `${style}${pokemon.name}</span>`;
  pokeHeight.innerHTML += `${style}${pokemon.height}</span>`;
  pokeWeight.innerHTML += `${style}${pokemon.weight}</span>`;

  // how to replace abilities???!
  let pokeAbilityArray = pokemon.abilities;

  for (let i = 0; i < pokeAbilityArray.length; i++) {
    abilityList.innerHTML += `<li style="font-weight: bold; color: darkred;">${pokeAbilityArray[i].ability.name}</li>`;
  }
